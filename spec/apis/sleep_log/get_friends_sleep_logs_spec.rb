...
RSpec.describe "GET /goodnight/api/v1/sleep_logs/friends", type: :request do
  include ActiveSupport::Testing::TimeHelpers

  let(:now) { Time.current }
  let!(:me) { User.create!(guid: SecureRandom.uuid, name: "Me") }
  let(:auth_token) do
    AuthService::Login.new(ActionController::Parameters.new(user_guid: me.guid)).call
  end
  let(:headers) { {
  ...
  context "when header authorization is empty" do
    it "return 401 unauthorized" do
      get "/goodnight/api/v1/sleep_logs/friends", headers: {}
  ...
  context "default behavior (last 7 days, ordered by duration desc)" do
    ...
    it "returns 200 and give followed users logs in last 7 days, sorted by longest duration" do
      get "/goodnight/api/v1/sleep_logs/friends", headers: headers

      logs = json["data"]["friend_sleep_logs"]
    ...
    it "returns 200 and the response matches with the expected data structure" do
      get "/goodnight/api/v1/sleep_logs/friends", params: { limit: 1, page: 1 }, headers: headers
